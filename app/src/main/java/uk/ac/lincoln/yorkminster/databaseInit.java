    ...
    private static class PopulateDb extends AsyncTask<Void, Void, Void> {
        ...
        private static void populateWithData(database db) {
            ...
            keyEntity key = new keyEntity(
                    0,"Nave Key", 53.221256,-0.54244572
            );
            addKey(db, key);

            key = new keyEntity(
                    1,"Striking Clock Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    2,"The Dragon Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    3,"The Fiddler Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    4,"South Transept and Rose Window Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    5,"North Quire Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    6,"Cathedra Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    7,"Central Tower Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    8,"The Chapter House Key", 0.1, 0.5
            );
            addKey(db, key);

            key = new keyEntity(
                    9, "Five Sisters Key", 0.1, 0.5
            );
            addKey(db, key);
        }
