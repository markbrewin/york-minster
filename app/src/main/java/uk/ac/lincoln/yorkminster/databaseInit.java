    ...
    private static class PopulateDb extends AsyncTask<Void, Void, Void> {
        ...
        private static void populateWithData(database db) {
            ...
            keyEntity key = new keyEntity(
                    "Nave Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "Striking Clock Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "The Dragon Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "The Fiddler Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "South Transept and Rose Window Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "North Quire Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "Cathedra Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "Central Tower Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "The Chapter House Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);

            key = new keyEntity(
                    "Five Sisters Key", 0.1, 0.5, 0.2
            );
            addKey(db, key);
        }
